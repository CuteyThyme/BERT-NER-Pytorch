                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if args.local_rank == -1:
                        ...
                        if current_score > best_score:
                            ...
                            torch.save(model.state_dict(), 'softmax_model_best_new.model')
                            traced_model = torch.jit.trace(model, [batch[0], batch[2], batch[1]])

                            torch.jit.save(traced_model, "softmax_best_model_new.pt")
                            logger.info('Saving best torchscript model')
                            print('best_f1: ', current_score)
...
def predict(args, model, tokenizer, prefix=""):
    ...
    for step, batch in enumerate(test_dataloader):
        batch = tuple(t.to(args.device) for t in batch)
        traced_model = torch.jit.trace(model, [batch[0], batch[2], batch[1]])
        torch.jit.save(traced_model, "softmax_best_model.pt")
        logger.info('Saving best torchscript model')
        model.eval()
        with torch.no_grad():
...
def main():
    ...
    config = config_class.from_pretrained(args.model_name_or_path, num_labels=num_labels, torchsctipt=True)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        tokenizer = tokenizer_class.from_pretrained(args.output_dir, do_lower_case=args.do_lower_case, torchscript=True)
